...
def rdmol_to_data(mol, name, R_G, P_G):
    ...
    data = Data(atom_type=z, pos=pos, edge_index=edge_index, edge_type=edge_type, is_alpha=None,
                rdmol=copy.deepcopy(mol), name=name, is_sidechain=None, atom2res=None, atom2alpha_index=None, RG=reac, PG=prod)
...
for Rind,Rxn in hf.items():
    ...
    data = rdmol_to_data(mol, Rsmiles, R_G, P_G)
    if data is not None:
...
# Write to pkl
with open('all_rgd_data_rpgeom.pkl', "wb") as fout:
...
val_size = 2
test_size = 1
with open("all_rgd_data_rpgeom.pkl", 'rb') as f:
...
with open("train_data_40k.pkl", "wb") as fout:
    pickle.dump(train_data, fout)
with open("val_data_5k.pkl", "wb") as fout:
    pickle.dump(val_data, fout)
with open("test_data_1k.pkl", 'wb') as fout:
    pickle.dump(test_data, fout)

# Check if created data correctly
with open("train_data_40k.pkl", "rb") as f:
...
train_set = ConformationDataset("train_data_40k.pkl", transform=transforms)
